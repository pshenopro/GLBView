...
export class Viewer {
	...
	setContent(object, clips) {
		...
		object.traverse(node => {
			if (['back', 'base_2001'].includes(node.name)) {
		...
		this.content.traverse((node) => {
			if (node.isLight) {
				...
				try {
					if (node.isSpotLight) {
						light = new SpotLight( Number('0x' + node.color?.getHexString()), node.intensity, node.distance, node.decay)
					}

					if (node.isPointLight) {
						light = new PointLight( Number('0x' + node.color?.getHexString()), node.intensity, node.distance, node.decay)
					}

					console.log('Цвет у света', node.type, node.color?.getHexString())
				} catch (e) {
					console.log('Ошибка копирования цвета у ' + node.type, e)
				}
				
				const { x, y, z } = node.position

				light.position.set( x, y, z );
				checkLights.push(node)
		...
		if (!checkLights.length) {
			console.log('Свет не обнаружен')
		} else {
			console.log('Свет передался', this.lights)
		}
